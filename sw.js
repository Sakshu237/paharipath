...
async function precacheHPTiles(client) {
  ...
  for (let i = 0; i < tiles.length; i += 10) {
    ...
    const results = await Promise.allSettled(batch.map(url =>
      fetch(url).then(r => { if (r.ok) { cache.put(url, r); return true; } return false; }).catch(() => false)
    ));
    cachedCount += results.filter(r => r.status === 'fulfilled' && r.value).length;
  }

  console.log(`PahariPath: Cached ${cachedCount} HP map tiles for offline use`);
  // Report back to the page so the UI can confirm this actually happened
  // instead of the result only ever showing up in the hidden SW console.
  if (client) {
    client.postMessage({ type: 'HP_TILES_CACHED', count: cachedCount, total: tiles.length });
  }
}
